...
export default function ProductInfo({ product, variants = [], activeVariant, showVariantCards = false, onVariantSelect }: ProductInfoProps) {
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {fabricOptions.map((variant) => {
            ...
            return onVariantSelect && variant.id ? (
              <button key={variant.id} type="button" onClick={() => onVariantSelect(variant.id)} className="text-left">
                {content}
              </button>
            ) : (
