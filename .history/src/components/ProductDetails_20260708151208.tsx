...
export default function ProductDetails({ product }: ProductDetailsProps) {
  ...
  const hasColorSelection = !product.inquiryOnly && product.colors.length > 0;

  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-10 px-4 pb-24 pt-28 sm:px-6 lg:px-8">
      <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
        <GarmentColorPreview imagePath={product.heroImage} colors={product.colors} selectedColor={selectedColor} onColorSelect={setSelectedColor} inquiryOnly={product.inquiryOnly} />
