...
const colorAssetAliases: Record<string, string> = {
  blue: "navy-blue",
};

const productSpecificColorAliases: Record<string, Record<string, string>> = {
  "oversized-t-shirts": {
    blue: "dark-blue",
  },
};
...
const getProductImage = (productSlug: string, selectedColour: string) => {
  ...
  const normalizedColour = selectedColour
    .toLowerCase()
    .replace(/\s+/g, "-");

  const productAlias = productSpecificColorAliases[productSlug]?.[normalizedColour];
  const alias = productAlias ?? colorAssetAliases[normalizedColour] ?? normalizedColour;

  return `/images/${prefix}-${alias}.jpg`;
};
...
export default function GarmentColorPreview({
  ...
  useEffect(() => {
    ...
    const generatedImage = productSlug ? getProductImage(productSlug, activeColor.name) : getProductImage(productPrefix, activeColor.name);
    const nextImage = generatedImage || selectedColorImage || imagePath;
