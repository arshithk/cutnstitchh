...
export default function GarmentColorPreview({ imagePath, colors, selectedColor, onColorSelect, inquiryOnly = false }: GarmentColorPreviewProps) {
  ...
  useEffect(() => {
    ...
    if (!image || !canvas || !activeColor) return;
  ...
  return (
    <div className="space-y-6">
      <motion.div
        ...
        className="overflow-hidden rounded-[2rem] border border-white/10 bg-black/80 p-3 shadow-[0_20px_70px_rgba(0,0,0,0.35)]"
      >
        <div className="relative aspect-4/5 overflow-hidden rounded-[1.5rem] border border-white/10 bg-black/70">
          <Image
            ref={imageRef}
            src={previewImage}
