...
export default function ProductCategories() {
  ...
  return (
    <section id="products" className="relative overflow-hidden bg-background py-16 sm:py-20 lg:py-24" ref={containerRef}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {categories.map((cat, idx) => (
            <motion.div
              ...
              className="bg-card border border-border-custom/60 rounded-2xl p-6 flex flex-col items-center text-center justify-between group hover:border-accent-custom/50 hover:shadow-md transition-all duration-300 relative overflow-hidden"
            >
              {/* Top info and vector outline */}
              <div className="flex flex-col items-center w-full">
                <div className="w-full aspect-square max-w-[150px] bg-foreground/[0.02] dark:bg-white/[0.02] rounded-xl flex items-center justify-center mb-6 group-hover:scale-105 group-hover:bg-accent-custom/5 transition-all duration-300 relative">
                  {cat.imagePath ? (
                    <img src={cat.imagePath} alt={cat.name} className="object-cover w-full h-full rounded-xl" />
                  ) : (
                    cat.svgPath
                  )}
                </div>
