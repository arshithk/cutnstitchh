        ...
        {related.map((product) => (
          <Link key={product.slug} href={`/products/${getCatalogCategorySlugByLegacyProductSlug(product.slug) ?? product.slug}`} className="group overflow-hidden rounded-3xl border border-white/10 bg-black/35 transition hover:-translate-y-1 hover:border-accent-custom/40">
            <div className="relative aspect-4/5 overflow-hidden">
              <Image src={normalizeImageSrc(product.heroImage)} alt={product.name} fill sizes="(max-width: 768px) 100vw, 25vw" className="object-cover transition duration-500 group-hover:scale-105" />
