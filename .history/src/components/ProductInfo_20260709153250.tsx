...
export default function ProductInfo({ product, variants = [], activeVariant, showVariantCards = false }: ProductInfoProps) {
  ...
  const isMultipleFabrics = fabricOptions.length > 1;
  ...
  return (
    <>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {fabricOptions.map((variant) => {
            ...
            return (
              <div
                key={variant.id}
                className={`relative flex h-full flex-col rounded-3xl border bg-white/5 p-5 text-left shadow-[0_18px_48px_rgba(0,0,0,0.18)] transition-all duration-300 ${isActive ? "border-[#d4af37] shadow-[0_0_0_15px_rgba(212,175,55,0.12)]" : "border-white/10 hover:border-white/20 hover:shadow-[0_24px_70px_rgba(0,0,0,0.14)]"}`}
              >
                {isActive ? (
      ...
      <section className="mt-8 grid gap-4 sm:grid-cols-2">
        ...
        {isMultipleFabrics ? null : (
          <div className="rounded-4xl border border-white/10 bg-black/35 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.22)]">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-muted-custom">Fabric</p>
            <p className="mt-3 text-lg font-semibold text-foreground">{product.fabric}</p>
          </div>
        )}
      </section>
    </>
