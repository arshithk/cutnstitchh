...
import { getCatalogCategorySlugByLegacyProductSlug, type ProductDetail } from "@/data/products";
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-border-custom/60 bg-card/80 shadow-[0_20px_50px_rgba(0,0,0,0.16)] transition-all duration-300 hover:-translate-y-1 hover:border-accent-custom/50 hover:shadow-[0_30px_70px_rgba(0,0,0,0.22)]">
      <div className="relative aspect-4/5 overflow-hidden">
        <Image
          src={normalizeImageSrc(product.heroImage)}
          alt={product.name}
          fill
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <Link href={`/products/${getCatalogCategorySlugByLegacyProductSlug(product.slug) ?? product.slug}`} className="mt-6 inline-flex items-center gap-2 rounded-full bg-accent-custom px-4 py-3 text-sm font-semibold text-black transition hover:brightness-110">
          View Products
