...
function blendHex(startHex: string, endHex: string, progress: number) {
  const start = hexToRgb(startHex);
  const end = hexToRgb(endHex);
  const mix = (startValue: number, endValue: number) => Math.round(startValue + (endValue - startValue) * progress);
...
export default function GarmentColorPreview({ imagePath, colors, selectedColor, onColorSelect, inquiryOnly = false }: GarmentColorPreviewProps) {
  ...
  const [renderedColor, setRenderedColor] = useState("#ffffff");
  const previousColorRef = useRef("#ffffff");
  ...
  useEffect(() => {
    if (!activeColor) {
      setRenderedColor("#ffffff");
      previousColorRef.current = "#ffffff";
      return;
    }

    const startColor = previousColorRef.current;
    const targetColor = activeColor.hex;
    ...
    const animate = (timestamp: number) => {
      ...
      const progress = Math.min((timestamp - startTime) / 220, 1);
      const eased = 1 - Math.pow(1 - progress, 3);
      setRenderedColor(blendHex(startColor, targetColor, eased));
      if (progress < 1) {
  ...
  useEffect(() => {
    ...
    if (image.complete) {
      render(renderedColor);
    } else {
      image.onload = () => render(renderedColor);
    }
  }, [activeColor, renderedColor, loaded, previewImage]);
