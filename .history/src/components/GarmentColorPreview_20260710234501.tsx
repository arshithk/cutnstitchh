...
import { getProductImageCandidates, resolveImagePath } from "@/lib/productImageMap";
...
export default function GarmentColorPreview({
  ...
  useEffect(() => {
    ...
    const loadPreview = async () => {
      ...
      const candidates = getProductImageCandidates(
        {
          categorySlug: productSlug || productName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, ""),
          productSlug,
          productName,
          fabric: activeColor.name,
        },
        activeColor.name,
        imagePath,
      );

      const resolvedImage = await resolveImagePath(candidates, imagePath);
      if (!isCancelled) {
  ...
  }, [activeColor.name, imagePath, productName, productSlug]);
