...
export default function GarmentColorPreview({
  ...
  useEffect(() => {
    const selectedColorImage = colors.find((color) => color.name === activeColor)?.imagePath;
    const nextImage = selectedColorImage || (productSlug ? getProductImage(productSlug, activeColor) : getProductImage(productPrefix, activeColor));
    setPreviewImage(nextImage || imagePath);
    setIsImageReady(true);
  }, [activeColor, colors, imagePath, productPrefix, productSlug]);
    ...
    <div className="space-y-6">
      ...
      >
        <div className="mx-auto flex w-full max-w-122 items-center justify-center px-0 py-0 sm:px-1 sm:py-1">
          <div className="group relative w-full overflow-hidden rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_top,rgba(212,175,55,0.15),transparent_55%),linear-gradient(145deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))] p-2 shadow-[0_24px_70px_rgba(0,0,0,0.28)] sm:p-3 lg:p-4">
            <div className="relative mx-auto aspect-[3/4] w-full max-w-[36rem] overflow-hidden rounded-[24px] bg-[#070707]">
              <motion.div
                key={previewImage}
                initial={{ opacity: 0, scale: 0.985 }}
                animate={{ opacity: isImageReady ? 1 : 0.65, scale: 1 }}
                transition={{ duration: 0.22, ease: "easeOut" }}
                className="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.05),transparent_70%)]"
              >
                <div className="relative h-[92%] w-[92%] max-w-[30rem]">
                  <Image
