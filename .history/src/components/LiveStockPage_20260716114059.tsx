...
// Get unique product type-GSM combinations
const getProductTypeGsmOptions = () => {
  const options = filteredStockEntries.map(
    (entry) => `${entry.productType} - ${entry.gsmRange}`
  );
  return Array.from(new Set(options)).sort();
};

const uniqueFabrics = () => {
  const fabrics = filteredStockEntries.map((entry) => entry.fabric);
  return Array.from(new Set(fabrics)).sort();
};

export default function LiveStockPage() {
  const [query, setQuery] = useState("");
  const [productTypeGsmFilter, setProductTypeGsmFilter] = useState("All");
  ...
  const filteredEntries = useMemo(() => {
    return filteredStockEntries.filter((entry) => {
      ...
      // Filter by product type and GSM combination
      const productTypeGsmLabel = `${entry.productType} - ${entry.gsmRange}`;
      const matchesProductTypeGsm =
        productTypeGsmFilter === "All" ||
        productTypeGsmLabel === productTypeGsmFilter;

      // Filter by fabric
      const matchesFabric =
        fabricFilter === "All" || entry.fabric === fabricFilter;

      return matchesQuery && matchesProductTypeGsm && matchesFabric;
    });
  }, [fabricFilter, productTypeGsmFilter, query]);
    ...
    <div className="mx-auto max-w-7xl px-4 pb-24 pt-28 sm:px-6 lg:px-8">
      <div className="rounded-4xl border border-slate-200 bg-[#F8FAFC] p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)] sm:p-8 lg:p-10 dark:border-slate-800/80 dark:bg-slate-900/95 dark:shadow-[0_24px_80px_rgba(2,6,23,0.55)] dark:backdrop-blur">
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-[1.2fr_0.8fr_0.8fr]">
          ...
          <select
            value={productTypeGsmFilter}
            onChange={(event) => setProductTypeGsmFilter(event.target.value)}
          ...
          >
            <option value="All">Product Type - GSM</option>
            {getProductTypeGsmOptions().map((option) => (
          ...
          <select
            ...
            className="rounded-full border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-[#D4AF37] focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/20 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-[#D4AF37]"
          >
            <option value="All">Fabric Type</option>
