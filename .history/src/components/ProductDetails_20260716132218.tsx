...
export default function ProductDetails({ product }: ProductDetailsProps) {
  ...
  const resolveDefaultColorName = (variant: ProductVariant | null) => {
    ...
    const heroMatchByImagePath = currentColors.find((color) =>
      normalizeImageSrc(color.imagePath ?? heroImage).toLowerCase() === normalizedHero,
    );
  ...
  const displayHeroImage = useMemo(() => {
    ...
    if (imageColorMatch && imageColorMatch.name.toLowerCase() === selectedColor.toLowerCase()) {
      return heroImage;
    }
