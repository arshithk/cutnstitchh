...
function selectVariantDisplayColor(colors: CatalogVariant["colors"]) {
  return (
    colors.find((color) => color.name.toLowerCase() !== "white" && color.name.toLowerCase() !== "off-white") ??
    colors[0]
  );
}

export default function VariantCard({ categorySlug, variant }: VariantCardProps) {
  const displayColor = selectVariantDisplayColor(variant.colors);
