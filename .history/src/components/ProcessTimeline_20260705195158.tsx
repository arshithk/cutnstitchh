...
export default function ProcessTimeline() {
  ...
  return (
    <section
      id="process"
      ref={sectionRef}
      className="relative overflow-hidden border-y border-border-custom/50 bg-card/40 py-16 sm:py-20 lg:py-24"
    >
      <div className="mx-auto flex max-w-7xl flex-col gap-12 px-4 sm:px-6 lg:px-8">
        ...
        <div
          ref={containerRef}
          className="flex gap-6 overflow-x-auto pb-8 pt-4 hide-scrollbar snap-x snap-mandatory cursor-grab active:cursor-grabbing"
        ...
        >
          {steps.map((step, idx) => (
            <motion.div
              ...
              className="flex-shrink-0 w-[300px] bg-background border border-border-custom p-7 rounded-2xl flex flex-col gap-6 shadow-sm snap-start relative group"
            >
              {/* Backside step numbering watermark */}
              <div className="absolute top-2 right-4 text-7xl font-black text-foreground/[0.03] group-hover:text-accent-custom/[0.05] transition-colors pointer-events-none">
                {step.no}
              </div>
