...
export default function ProductDetails({ product }: ProductDetailsProps) {
  ...
  const selectedColorData = useMemo(() => {
    return displayProduct.colors.find((color) => color.name === selectedColor) ?? displayProduct.colors[0];
  }, [displayProduct.colors, selectedColor]);

  const selectedColorHex = selectedColorData?.hex ?? "#d4af37";
  const pageBackgroundStyle = useMemo(() => ({
    backgroundImage: `radial-gradient(circle at top left, ${hexToRgba(selectedColorHex, 0.24)} 0%, transparent 42%), linear-gradient(135deg, ${hexToRgba(selectedColorHex, 0.14)} 0%, rgba(5, 8, 16, 0.96) 60%)`,
  }), [selectedColorHex]);
