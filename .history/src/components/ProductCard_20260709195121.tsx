...
export default function ProductCard({ product, category }: ProductCardProps) {
  const isCategoryCard = Boolean(category);
  const title = product?.name ?? category?.name ?? "Product";
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-border-custom/60 bg-card/80 shadow-[0_20px_50px_rgba(0,0,0,0.16)] transition-all duration-300 hover:-translate-y-1 hover:border-accent-custom/50 hover:shadow-[0_30px_70px_rgba(0,0,0,0.22)]">
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-3 text-sm leading-6 text-muted-custom">{description}</p>
        <div className="mt-4 flex flex-wrap gap-2">
          {isCategoryCard ? (
            <span className="rounded-full border border-border-custom/70 px-2.5 py-1 text-xs text-muted-custom">
              {category?.variants.length ?? 0} Fabric Variants
            </span>
          ) : (
            <>
