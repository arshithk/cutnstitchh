...
function buildAvailableColors(colors: ProductColor[], fallbackImagePath: string, context?: ProductImageContext): ProductColor[] {
  ...
  // Special cases: these variants should only show their explicit colors
  ...
  const isSweatshirtCottonFleece = 
    context?.variantSlug?.includes("sweatshirt") && context?.variantSlug?.includes("fleece");

  let baseColors: ProductColor[];
  if ((is340GSMHoodie || isSweatshirtCottonFleece) && colors.length > 0) {
    // For 340GSM hoodies and sweatshirt-cotton-fleece, use only their explicit colors (filtered by palette)
    ...
    // For all other products, always use the full allowed palette
