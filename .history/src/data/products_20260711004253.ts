...
function buildAvailableColors(colors: ProductColor[], fallbackImagePath: string, scope?: string, context?: ProductImageContext): ProductColor[] {
  if (colors.length > 0) {
...
export const catalogCategories: CatalogCategory[] = baseCatalogCategories.map((category) => ({
  ...
  variants: category.variants.map((variant) => ({
    ...variant,
    colors: buildAvailableColors(variant.colors, variant.colors[0]?.imagePath ?? variant.heroImage, category.slug, {
...
export const products: ProductDetail[] = baseProducts.map((product) => {
  ...
  return {
    ...product,
    colors: buildAvailableColors(product.colors, product.colors[0]?.imagePath ?? product.heroImage, product.slug, {
    ...
    variants: product.variants?.map((variant) => ({
      ...variant,
      colors: buildAvailableColors(variant.colors, variant.colors[0]?.imagePath ?? variant.heroImage, product.slug, {
        categorySlug: product.slug,
        productSlug: product.slug,
        productName: product.name,
        fabric: variant.fabric,
