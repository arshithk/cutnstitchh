...
function buildAvailableColors(imagePath: string, scope?: string): ProductColor[] {
  const palette = scope === "shorts" || scope === "joggers" ? shortsAndJoggersColorPalette : standardAvailableColorPalette;
...
export const catalogCategories: CatalogCategory[] = baseCatalogCategories.map((category) => ({
  ...
  variants: category.variants.map((variant) => ({
    ...variant,
    colors: buildAvailableColors(variant.colors[0]?.imagePath ?? variant.heroImage, category.slug),
  })),
}));
...
export const products: ProductDetail[] = baseProducts.map((product) => {
  ...
  return {
    ...product,
    colors: buildAvailableColors(product.colors[0]?.imagePath ?? product.heroImage, product.slug),
    variants: product.variants?.map((variant) => ({
      ...variant,
      colors: buildAvailableColors(variant.colors[0]?.imagePath ?? variant.heroImage, product.slug),
    })),
  };
});
