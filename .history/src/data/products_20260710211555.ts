...
function buildStandardAvailableColors(imagePath: string): ProductColor[] {
  return standardAvailableColorPalette.map((color) => ({
    ...color,
    imagePath,
  }));
}
...
export const catalogCategories: CatalogCategory[] = baseCatalogCategories.map((category) => ({
  ...
  variants: category.variants.map((variant) => ({
    ...variant,
    colors: buildStandardAvailableColors(variant.colors[0]?.imagePath ?? variant.heroImage),
  })),
}));
...
export const products: ProductDetail[] = baseProducts.map((product) => {
  ...
  return {
    ...product,
    colors: buildStandardAvailableColors(product.colors[0]?.imagePath ?? product.heroImage),
