...
function buildAvailableColors(colors: ProductColor[], fallbackImagePath: string, context?: ProductImageContext): ProductColor[] {
  ...
  // Filter explicit colors to only include those in the allowed palette for this category
  const filteredColors = colors.filter((color) => paletteNames.has(color.name));

  // Use filtered colors if any remain; otherwise use the full palette
  const baseColors = filteredColors.length > 0 ? filteredColors : palette;

  return baseColors.map((color) => ({
    ...color,
    imagePath:
      (color as ProductColor).imagePath ||
      (context
