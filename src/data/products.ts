...
function enrichDescription(text: string, fabric?: string, extraKeywords?: string[]) {
  const base = text.trim();

  // If the description already appears enriched, skip
  if (base.includes("Made in India") || base.includes("DTF Printing")) return base;

  const kws = new Set<string>();

  // Include DTF for broad compatibility
...
// Product-specific keyword lists (industry-accurate)
const KEYWORDS_PREMIUM_COTTON_180 = [
  "100% Premium Cotton",
  "180 GSM",
  "Bio Washed Fabric",
  "Super Combed Cotton",
...
const KEYWORDS_POLY_140 = [
  "Premium Polyester",
  "140 GSM",
  "Heavy Performance Fabric",
  "Quick Dry",
  "Moisture Wicking",
  "Breathable",
  "Durable",
...
function getProductKeywords(variant: CatalogVariant): string[] {
  ...
  if (fabric.includes("cotton") && gsm.includes("180") && name.includes("round")) return KEYWORDS_PREMIUM_COTTON_180;
  if (fabric.includes("polycotton") && gsm.includes("180")) return KEYWORDS_POLYCOTTON_180;
  if (fabric.includes("polyester") && gsm.includes("90")) return KEYWORDS_POLY_90;
  if (fabric.includes("polyester") && gsm.includes("110")) return KEYWORDS_POLY_110;
  if (fabric.includes("polyester") && gsm.includes("140")) return KEYWORDS_POLY_140;
  if (name.includes("dri fit") || name.includes("dri-fit") || name.includes("dri fit mars")) return KEYWORDS_DRI_FIT_180;
  if (name.includes("honeycomb") || name.includes("dot knit") || gsm.includes("160") && name.includes("polo")) return KEYWORDS_HONEYCOMB_160;
  if (name.includes("saleena") || name.includes("selena")) return KEYWORDS_SALEENA_160;
  if (name.includes("polo") && fabric.includes("polycotton")) return KEYWORDS_POLY_Polo_220;
  if (name.includes("polo") && fabric.includes("cotton")) return KEYWORDS_COTTON_POLO;
  if (name.includes("hoodie") && fabric.includes("polycotton")) return KEYWORDS_POLY_HOODIES;
  if (name.includes("hoodie") && fabric.includes("cotton")) return KEYWORDS_COTTON_HOODIES;
  if (variant.categorySlug === "oversized" || name.includes("oversized")) return KEYWORDS_OVERSIZED;

  // default: return empty (generic enrichment will still apply)
  return [];
}
...
function parseGsmValue(gsmRange?: string): number {
  if (!gsmRange) return Number.MAX_SAFE_INTEGER;
  const match = gsmRange.match(/(\d{2,3})/);
  if (!match) return Number.MAX_SAFE_INTEGER;
  return Number.parseInt(match[1], 10);
}

function getFabricPriority(fabric: string): number {
...
function sortVariantsByFabricAndGsm<T extends { fabric: string; gsm?: string; gsmRange?: string; name: string }>(variants?: T[]) {
...
function buildAvailableColors(colors: ProductColor[], fallbackImagePath: string, context?: ProductImageContext): ProductColor[] {
  ...
  let baseColors: ProductColor[];
  if (is340GSMHoodie && colors.length > 0) {
    // For 340GSM hoodies, use only the explicit variant colors.
  ...
  return baseColors.map((color) => ({
    ...
    imagePath:
      color.imagePath ||
      (context
        ? getProductImagePath(context, color.name, fallbackImagePath)
        : fallbackImagePath),
  }));
}
    ...
    variants: [
      {
        ...
        heroImage: "/images/cotton-regular-fit-tshirt-red.jpeg",
        thumbnailImage: "/images/cotton-regular-fit-tshirt-red.jpeg",
        colors: [
          { name: "White", hex: "#f7f7f2", imagePath: "/images/cotton-regular-fit-tshirt-white.jpeg" },
          { name: "Black", hex: "#111111", imagePath: "/images/cotton-regular-fit-tshirt-black.jpeg" },
          { name: "Navy Blue", hex: "#20354d", imagePath: "/images/cotton-regular-fit-tshirt-navy-blue.jpeg" },
        ],
      ...
      
      {
        ...
        heroImage: "/images/cotton-regular-fit-tshirt-grey.jpeg",
        thumbnailImage: "/images/cotton-regular-fit-tshirt-grey.jpeg",
        colors: [
          { name: "White", hex: "#f7f7f2", imagePath: "/images/cotton-regular-fit-tshirt-white.jpeg" },
          { name: "Grey", hex: "#6b7280", imagePath: "/images/cotton-regular-fit-tshirt-grey.jpeg" },
          { name: "Maroon", hex: "#6d2c2c", imagePath: "/images/cotton-regular-fit-tshirt-maroon.jpeg" },
        ],
        sizes: ["S", "M", "L", "XL", "2XL", "3XL"],
        moq: "100 Pieces",
      ...
      },
      {
        ...
        colors: [
          ...
          { name: "Royal Blue", hex: "#1f3b64", imagePath: "/images/regular-fit-tshirt-dark-blue.jpg" },
        ],
        sizes: ["S", "M", "L", "XL", "2XL", "3XL"],
        moq: "100 Pieces",
  ...
  },
  {
    ...
    variants: [
      ...
      },
      {
        ...
        moq: "100 Pieces",
        pricing: [
          { min: 100, max: 1000, price: 89 },
          { min: 1001, max: 5000, price: 88 },
          { min: 5001, price: 87 },
        ],
    ...
    variants: [
      {
        ...
        name: "Cotton Fleece 300 GSM Hoodie with full zip",
        gsm: "300 GSM",
      ...
      },
      {
        ...
        name: "Cotton Fleece 360 GSM Hoodie with full zip",
        gsm: "360 GSM",
        fabric: "100% Cotton Fleece",
        fit: "Regular Fit",
      ...
      },
      {
        ...
        name: "PolyCotton Fleece 300 GSM Hoodie with full zip",
        gsm: "300 GSM",
      ...
      },
      {
        ...
        heroImage: "/images/hoodie-without-zip-black.png",
        thumbnailImage: "/images/hoodie-without-zip-black.png",
        colors: [
          { name: "Black", hex: "#111111", imagePath: "/images/hoodie-without-zip-black.png" },
          { name: "Navy Blue", hex: "#20354d", imagePath: "/images/hoodie-without-zip-dark-blue.jpeg" },
          { name: "Grey", hex: "#6b7280", imagePath: "/images/hoodie-without-zip-grey.png" },
        ],
        sizes: ["S", "M", "L", "XL", "2XL"],
        moq: "100 Pieces",
        pricing: [
  ...
  },
  {
    ...
    variants: [
      ...
      },
      {
        ...
        thumbnailImage: "/images/sweatshirt-cotton-fleece-white.png",
        colors: [
          { name: "White", hex: "#f5f5f2", imagePath: "/images/sweatshirt-cotton-fleece-white.png" },
          { name: "Black", hex: "#111111", imagePath: "/images/sweatshirt-cotton-fleece-black.png" },
          { name: "Navy Blue", hex: "#22334d", imagePath: "/images/sweatshirt-cotton-fleece-navy-blue.png" },
          { name: "Royal Blue", hex: "#1f3b64", imagePath: "/images/sweatshirt-cotton-fleece-dark-blue.png" },
          { name: "Maroon", hex: "#6d2c2c", imagePath: "/images/sweatshirt-cotton-fleece-maroon.jpeg" },
          { name: "Orange", hex: "#c96a17", imagePath: "/images/sweatshirt-cotton-fleece-orange.jpeg" },
          { name: "Red", hex: "#a52424", imagePath: "/images/sweatshirt-cotton-fleece-red.jpeg" },
          { name: "Brown", hex: "#6f4b2f", imagePath: "/images/sweatshirt-cotton-fleece-brown.png" },
          { name: "Grey", hex: "#6b7280", imagePath: "/images/sweatshirt-cotton-fleece-grey.png" },
        ],
        sizes: ["S", "M", "L", "XL", "2XL"],
        moq: "100 Pieces",
  ...
  },
  {
    ...
    variants: [
      ...
      },
      {
        ...
        pricing: [
          { min: 100, max: 999, price: 149 },
          { min: 1000, max: 5000, price: 147 },
          { min: 5001, price: 145 },
        ],
  ...
  },
  {
    ...
    variants: [
      ...
      },
      {
        ...
        pricing: [
          { min: 100, max: 999, price: 160 },
          { min: 1000, max: 5000, price: 158 },
          { min: 5001, price: 155 },
        ],
      ...
      },
      {
        ...
        embroideryCompatibility: "Supports premium logo detailing with stretch-friendly structure.",
        pricing: [
          { min: 100, max: 999, price: 199 },
          { min: 1000, max: 5000, price: 198 },
          { min: 5001, price: 195 },
        ],
...
export const catalogCategories: CatalogCategory[] = baseCatalogCategories.map((category) => ({
  ...
  variants: (sortVariantsByFabricAndGsm(category.variants) ?? []).map((variant) => {
    const shouldKeep = category.slug === "joggers" || category.slug === "shorts";
...
const baseProducts: ProductDetail[] = [
  {
    ...
    colors: [
      ...
      { name: "Royal Blue", hex: "#1f3b64", imagePath: "/images/oversized tshirt.jpg" },
