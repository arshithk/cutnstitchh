...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setStatus("error");
        setSubmissionError(result?.error ?? "Unable to submit inquiry. Please try again.");
        return;
      }
      ...
      console.error(err);
      setStatus("error");
      setSubmissionError("Unable to submit inquiry. Please try again later.");
    }
  };
          ...
          <div className="rounded-3xl border border-border-custom/60 bg-background p-6 shadow-md sm:p-8 lg:col-span-7">
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                {submissionError ? (
                  <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-700/50 dark:bg-red-900/30 dark:text-red-200">
                    {submissionError}
                  </div>
                ) : null}
